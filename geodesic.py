...
def geodesic( r, f ):
    '''
    Returns a list of polygons, edges and vertices that
    make up a geodesic sphere of radius r and frequency f.
    '''
    def tessellate( p, f ):
        ''' Tessellates the vertices in p a frequency of f times. '''
        for i in range( f ):
    ...
    def project( p, r ):
        '''
        Projects the vertices in p onto the surface of a sphere
        of radius r.
        '''
    ...
    tessellation   = tessellate( icosahedron, f )
    edges         = [ project( edge, r ) for edge in edges( tessellation, f ) ]
    triangulation = polygons ( tessellation, f )
    projection    = [ project( polygon, r ) for polygon in triangulation ]
    return projection, edges, project( tessellation, r )

polys, edges, p = geodesic( 1, 3 )
x = p[:,0]
y = p[:,1]
z = p[:,2]
